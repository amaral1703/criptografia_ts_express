...
export function hillValidateEncryptionInput(text: any, key: any): ValidationResult {
  ...
  // Verificação se a mensagem é uma palavra ou chave
  if (text.trim().length < 1) {
    return { isValid: false, error: 'A mensagem deve ser uma palavra ou chave não vazia' };
  }

  // Verificação se a chave é uma palavra ou chave
  if (key.trim().length < 1) {
    return { isValid: false, error: 'A chave deve ser uma palavra ou chave não vazia' };
  }
